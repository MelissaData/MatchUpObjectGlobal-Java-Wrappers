...
$Files = @(
  ...
  [FileConfig]@{
    FileName       = "mdMatchupJavaWrapper.cpp";
    ReleaseVersion = $RELEASE_VERSION;
    OS             = "ANY";
    Compiler       = "JAVA";
    Architecture   = "ANY";
...
function DownloadFiles() {
  ...
  if ($File.FileName -eq "mdMatchup_JavaCode.zip") {
    if (!(Test-Path ("$CurrentPath\mdMatchup_JavaCode.zip"))) {
      ...
      Expand-Archive -Path "$CurrentPath\mdMatchup_JavaCode.zip" -DestinationPath $CurrentPath
      }
      else {
...
function CheckFiles() {
  ...
  $FileMissing = $false 
  # if (!(Test-Path ("mdMatchup.dll"))) {
  #   Write-Host "mdMatchup.dll not found." 
  #   $FileMissing = $true
  # }
  if (!(Test-Path ("mdMatchupEnums.h"))) {
    Write-Host "mdMatchupEnums.h not found." 
    $FileMissing = $true
  }
  if (!(Test-Path ("mdMatchup.h"))) {
    Write-Host "mdMatchup.h not found." 
    $FileMissing = $true
  }
  if (!(Test-Path ("mdMatchup.lib"))) {
    Write-Host "mdMatchup.lib not found." 
    $FileMissing = $true
  }
  if ($FileMissing) {
...
Write-Host "`n======================= WELCOME TO MELISSA MATCHUP OBJECT GLOBAL JAVA WRAPPER =======================`n"
...
jar cvf mdMatchup.jar com\melissadata\*.class > $null

if (Test-Path ("mdMatchupJavaWrapper.dll")) {
  $filePath = Join-Path -Path (Split-Path -Parent $PSCommandPath) -ChildPath "mdMatchupJavaWrapper.dll"

  Write-Host "`nmdMatchupJavaWrapper.dll has been successfully generated at:`n$filePath"
}
else {
  Write-Host "`nError: was not able to create mdMatchupJavaWrapper.dll"
}

if (Test-Path ("mdMatchup.jar")) {
  $filePath = Join-Path -Path (Split-Path -Parent $PSCommandPath) -ChildPath "mdMatchup.jar"

  Write-Host "`nmdMatchup.jar has been successfully generated at:`n$filePath"
}
else {
  Write-Host "`nError: was not able to create mdMatchup.jar"
}
