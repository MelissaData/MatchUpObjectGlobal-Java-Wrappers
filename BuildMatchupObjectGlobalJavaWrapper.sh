...
Config_FileName1="libmdMatchup.so"
...
Config_FileName2="mdMatchupEnums.h"
...
Config_FileName4="mdMatchupJavaWrapper.cpp"
...
########################## Main ############################
printf "\n======================= WELCOME TO MELISSA MATCHUP OBJECT GLOBAL JAVA WRAPPER =======================\n"
...
# Compile SO
gcc -fpic -c -I$ProjectPath -I$JavaIncludeDirectory1 -I$JavaIncludeDirectory2 mdMatchupJavaWrapper.cpp
gcc -g -shared mdMatchupJavaWrapper.o -o libmdMatchupJavaWrapper.so -L$ProjectPath -lmdMatchup -pthread

rm mdMatchupJavaWrapper.o

# Compile Jar
javac com/melissadata/*.java &> /dev/null

jar cvf mdMatchup.jar com/melissadata/*.class &> /dev/null

if [ -f "libmdMatchupJavaWrapper.so" ];
then
  script_dir="$( cd "$( dirname "${BASH_SOURCE[0]}" )" &> /dev/null && pwd )"
  file_path="$script_dir/libmdMatchupJavaWrapper.so"

  printf "\nlibmdMatchupJavaWrapper.so has been successfully generated at:\n$file_path\n"
else
  printf "\nError: was not able to create libmdMatchupJavaWrapper.so\n"
fi

if [ -f "mdMatchup.jar" ];
then
  script_dir="$( cd "$( dirname "${BASH_SOURCE[0]}" )" &> /dev/null && pwd )"
  file_path="$script_dir/mdMatchup.jar"

  printf "\nmdMatchup.jar has been successfully generated at:\n$file_path\n"
else
  printf "\nError: was not able to create mdMatchup.jar\n"
fi
